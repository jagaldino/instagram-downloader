...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!url) {
      return res
        .status(400)
        .json({ message: "URL não fornecida", success: false });
    }
    ...
    if (!isValidInstagramUrl(url)) {
      return res
        .status(400)
        .json({ message: "URL do Instagram inválida", success: false });
    }
    ...
    try {
      ...
      console.error("Erro ao obter mídia do Instagram:", fetchError);
      return res.status(422).json({
        message:
          "Não foi possível extrair a mídia deste URL do Instagram. O post pode ser privado ou não existir.",
        success: false,
    ...
    return res.status(500).json({
      message:
        "Erro interno ao processar a requisição. Tente novamente mais tarde.",
      success: false,
...
function isValidInstagramUrl(url) {
  const regex =
    /^https?:\/\/(www\.)?instagram\.com\/(p|reel|stories)\/[^\/]+\/?/;
...
async function fetchInstagramMedia(url) {
  try {
    ...
    const response = await axios.get(url, {
      ...
      timeout: 10000,
      // Não seguir redirecionamentos para evitar páginas de login
      maxRedirects: 0,
    ...
    const ogTitle =
      $('meta[property="og:title"]').attr("content") || "Instagram Media";
      ...
      try {
        ...
        if (scriptContent) {
          ...
          if (jsonData && jsonData.video) {
            ...
            mediaUrl = Array.isArray(jsonData.image)
    ...
    if (!mediaUrl) {
      const sharedDataScript = $(
        'script:contains("window._sharedData")'
      ).html();
      if (sharedDataScript) {
        const sharedDataMatch = sharedDataScript.match(
          /window\._sharedData = (.+);/
        );
    ...
    if (!mediaUrl) {
      throw new Error(
        "Não foi possível encontrar a mídia neste URL. Verifique se o post é público e existe."
      );
    }

    // Verificar se a URL da mídia é válida
    if (!mediaUrl.startsWith("http")) {
      throw new Error(
        "URL da mídia inválida. O Instagram pode ter alterado sua estrutura."
      );
    }
    ...
    const filename = `instagram-${postId}.${
    ...
    throw new Error(
      "Não foi possível obter a mídia do Instagram. Verifique se o URL é válido."
    );
  }
}
