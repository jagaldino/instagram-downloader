document.addEventListener("DOMContentLoaded", () => {
  ...
  function showError(message) {
    ...
    errorMessage.textContent =
      message ||
      "Ocorreu um erro. Verifique se o URL é válido e tente novamente.";
  ...
  async function processInstagramUrl(url) {
    if (requestCount >= MAX_REQUESTS) {
      return showError(
        "Você atingiu o limite de downloads. Tente novamente mais tarde."
      );
    }
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "A resposta do servidor não é um JSON válido. Tente novamente mais tarde."
        );
      }

      // Verificar se a resposta indica sucesso
      if (!response.ok || !data.success) {
        throw new Error(data.message || "Erro ao processar o URL do Instagram");
      }

      // Verificar se a URL da mídia está presente
      if (!data.url) {
        throw new Error("Não foi possível obter a mídia do Instagram");
      }
